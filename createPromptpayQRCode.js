...
const createPromptpayQRCode = (qrtype, accountNo, accountName, totalCharge, apiSubLink, filename, id, callback = null) => {
	const creatorName = (apiSubLink === "line")? "Mr.QR" : "F4SME";
	const creatorLink = (apiSubLink === "line")? "เพิ่ม Mr.QR เป็นเพื่อน" : "ที่ m.me/f4sme";
	...
	ctx.fillStyle = 'black';
	ctx.textAlign = 'center';
	ctx.fillText("คนไทยทุกคนสร้างพร้อมเพย์คิวอาร์โค้ดได้ฟรี ", 200, 550);
	...
	ctx.drawImage(mrqrLogoImage, 10, 560, 100, 100);
	...
	ctx.drawImage(mrqrImage, 290, 560, 100, 100);

	var QRText = qrcodetextgen.generateQRCodeText(qrtype, accountNo, totalCharge); 
		
	logger().info(qrtype  + " >> " + accountNo  + " >> " + totalCharge + " >> " + id + " >> " + QRText + " >> " + new Date());
